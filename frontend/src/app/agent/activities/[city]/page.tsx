                    ...
                    <div className="flex items-center justify-between">
                        <div className="flex items-center gap-5">
                            <Button
                                ...
                                className="h-11 w-11 shrink-0 rounded-full bg-white/40 backdrop-blur-md border border-white/50 hover:bg-[#FF6B2B] hover:text-white hover:border-[#FF6B2B] text-slate-600 shadow-sm transition-all duration-300"
                            >
                                <ArrowLeft className="h-5 w-5" />
                            </Button>
                            <div>
                                <h1 className="text-3xl font-bold flex items-center gap-3 tracking-tight" style={{ fontFamily: "'Playfair Display', serif", color: '#4A2B1D' }}>
                    ...
                    <div className="h-full flex flex-col overflow-hidden border border-white/40 shadow-sm transition-all" style={{ background: 'rgba(255,255,255,0.18)', backdropFilter: 'blur(24px)', WebkitBackdropFilter: 'blur(24px)', borderRadius: '32px' }}>
                        <div className="p-6 shrink-0 sticky top-0 z-10 border-b border-white/40 bg-white/20 backdrop-blur-md">
                            <div className="flex items-center justify-between mb-2">
                                <h2 className="text-xl font-bold text-[#4A2B1D]">Existing Activities</h2>
                        ...
                        <div className="p-6 pt-4 space-y-4 overflow-y-auto custom-scrollbar flex-1 relative">
                            {isLoading ? (
                                <div className="flex justify-center p-8">
                                    <div className="animate-spin rounded-full h-8 w-8 border-2 border-[#FF6B2B]/20 border-t-[#FF6B2B]"></div>
                                </div>
                            ) : existingActivities.length === 0 ? (
                                <div className="text-center p-8 border-2 border-dashed border-[#FF6B2B]/30 rounded-[24px] bg-white/30 backdrop-blur-sm shadow-sm">
                                    ...
                                    <p className="text-[#4A2B1D] text-[15px] font-semibold mb-1">No activities planned yet.</p>
                                        ...
                                        <Card key={activity.id} className={`overflow-hidden relative group border-white/50 hover:shadow-[0_12px_30px_rgba(255,107,43,0.1)] transition-all duration-300 hover:-translate-y-1 ${isEditing ? 'border-l-4 border-l-[#FF6B2B] bg-[#FF6B2B]/10 shadow-[0_8px_24px_rgba(255,107,43,0.15)] scale-[1.02]' : ''}`} style={{ background: isEditing ? 'rgba(255,107,43,0.08)' : 'rgba(255,255,255,0.4)', backdropFilter: 'blur(20px)', borderRadius: '20px' }}>
                                            <div className="h-40 relative rounded-t-[20px] overflow-hidden">
                                                ...
                                                <div className="absolute top-3 right-3 opacity-100 group-hover:opacity-100 transition-all duration-300 z-20 flex gap-2">
                                                    <Button size="icon" variant="ghost" className={`h-9 w-9 rounded-full backdrop-blur-md border shadow-sm transition-all ${isEditing ? 'bg-[#FF6B2B] text-white border-[#FF6B2B]' : 'bg-white/40 text-slate-700 border-white/50 hover:bg-white/60 hover:text-[#FF6B2B]'}`} onClick={() => handleEditClick(activity)}>
                                                ...
                                                <div className="absolute bottom-0 inset-x-0 bg-white/40 backdrop-blur-md p-3 px-4 border-t border-white/50 flex justify-between items-center bg-gradient-to-t from-white/60 to-white/10 z-10">
                                                    <h3 className="font-bold text-[#4A2B1D] text-[17px] line-clamp-1 flex-1 tracking-tight" style={{ fontFamily: "'Playfair Display', serif" }}>{activity.name}</h3>
                                            ...
                                            <CardContent className="p-4 pt-3 flex flex-wrap gap-2.5 z-20 relative">
                                                ...
                                                <span className="flex items-center gap-1.5 text-slate-700 font-medium text-xs bg-white/60 border border-white/50 shadow-sm px-2.5 py-1 rounded-full backdrop-blur-md">
                                                    <Clock className="h-3 w-3 text-[#FF6B2B]" /> {activity.duration_hours}h
                                                </span>
                    ...
                    <div className="max-w-4xl mx-auto space-y-8 pb-20">
                        {activityRows.map((row, index) => (
                            <div key={index} className="glass-panel relative overflow-visible animate-in fade-in slide-in-from-bottom-4 group">
                                ...
                                <div className={`absolute -left-4 -top-4 bg-gradient-to-br transition-all duration-500 w-10 h-10 rounded-full flex items-center justify-center font-bold text-sm shadow-lg border-2 border-white/90 z-10 group-hover:scale-110 ${editingActivity ? 'from-[#FF6B2B] to-[#D95D22] text-white scale-110' : 'from-[#FF9A5C] to-[#FF6B2B] text-white'}`}>
                                    {editingActivity ? <Edit className="h-5 w-5" /> : index + 1}
                                </div>

                                {editingActivity && (
                                    <div className="absolute left-10 -top-3.5 px-4 py-1.5 bg-white/40 backdrop-blur-md border border-white/60 rounded-full text-[#4A2B1D] text-xs font-bold shadow-sm animate-in fade-in slide-in-from-left-2">
                                        Editing: <span className="text-[#FF6B2B]">{editingActivity.name}</span>
                                    </div>
                                )}
                                ...
                                <CardContent className="p-8">
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-7">
                                        ...
                                        <div>
                                            ...
                                            >
                                                <SelectTrigger className="bg-white/22 backdrop-blur-md border border-white/40 rounded-[14px] text-slate-800 focus:ring-0 focus:border-[#FF6B2B]/50 hover:bg-white/30 transition-all font-medium text-[15px] h-10 shadow-sm data-[placeholder]:text-slate-500/80">
                                    ...
                                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-7 mt-7">
                                        ...
                                        <div className="floating-label-group">
                                            <div className="relative flex items-center h-10">
                                                <Input
                                                    ...
                                                    placeholder=" "
                                                />
                                                <div className="absolute right-1.5 top-1/2 -translate-y-1/2 bg-white/50 backdrop-blur-md border border-white/60 px-2.5 py-1 rounded-[10px] text-[#FF6B2B] text-xs font-bold pointer-events-none shadow-sm">
                                                    hrs
                                                </div>
                                            </div>
                                        ...
                                        <div className="floating-label-group">
                                            <div className="relative h-10">
                                                <div className="absolute left-3.5 top-1/2 -translate-y-1/2 text-[#FF6B2B] font-bold">₹</div>
                                                <Input
                                                    type="number"
                                                    min="0"
                                    ...
                                    <div className="mt-8 space-y-4 p-5 bg-white/10 backdrop-blur-md border-2 border-dashed border-[#FF6B2B]/40 rounded-[16px] shadow-sm relative transition-all hover:border-[#FF6B2B]/60 hover:bg-white/20">
                                        <div className="flex items-center justify-between">
                                            <Label className="text-[#4A2B1D] font-bold flex items-center gap-2.5 text-base">
                                                <div className="bg-orange-100 p-1.5 rounded-lg border border-orange-200">
                                                    <Upload className="h-4 w-4 text-[#FF6B2B]" />
                                                </div>
                                                Activity Images
                                            ...
                                            <div className="flex gap-2.5">
                                                <Button
                                                    ...
                                                    className="glass-button h-9 px-4 text-xs font-bold rounded-full text-[#D95D22] border border-[#FF6B2B]/30 hover:bg-[#FF6B2B]/10 hover:border-[#FF6B2B]/50 transition-all shadow-sm"
                                        ...
                                        {row.images && row.images.length > 0 && (
                                            <div className="flex gap-3 overflow-x-auto pb-2 custom-scrollbar">
                                                {row.images.map((img: any, imgIdx: number) => (
                                                    <div key={imgIdx} className={`relative group shrink-0 h-28 w-40 rounded-xl overflow-hidden border-2 flex items-center justify-center bg-white/20 ${imgIdx === 0 ? 'border-[#FF6B2B] shadow-sm' : 'border-white/40'}`}>
                                                        {img.image_url === 'loading' ? (
                                                            <div className="animate-spin rounded-full h-6 w-6 border-b-2 border-[#FF6B2B]"></div>
                                                        ) : (
                                                            <>
                                                                ...
                                                                {/* Primary Badge */}
                                                                {imgIdx === 0 && (
                                                                    <div className="absolute top-0 left-0 bg-gradient-to-r from-[#FF9A5C] to-[#FF6B2B] text-white text-[10px] font-bold px-2 py-1 rounded-br-lg shadow-sm z-10">
                                                                        PRIMARY
                                                                    </div>
                                                                )}
                        ...
                        <div className="flex flex-col items-center gap-4 pt-2">
                            {editingActivity ? (
                                ...
                                <Button
                                    ...
                                    variant="outline"
                                    className="w-full max-w-2xl glass-button rounded-[50px] border-2 border-dashed border-[#FF6B2B]/50 text-[#FF6B2B] hover:bg-[#FF6B2B]/10 hover:border-[#FF6B2B] transition-all duration-300 h-14 font-bold text-base flex items-center justify-center gap-3 shadow-sm bg-white/20"
                                >
                                    <div className="bg-[#FF6B2B]/20 rounded-full p-1 border border-[#FF6B2B]/30 shadow-inner">
