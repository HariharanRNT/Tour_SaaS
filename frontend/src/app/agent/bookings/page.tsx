    ...
    return (
        <Card className="relative overflow-hidden transition-all duration-500 hover:-translate-y-1 group border-white/40 shadow-[0_16px_48px_rgba(0,0,0,0.05)] hover:shadow-[0_20px_60px_var(--primary-glow)]"
            ...
            <div className="flex flex-col md:flex-row min-h-[180px]">
                ...
                <div className="relative w-full h-48 md:h-auto md:w-64 shrink-0 overflow-hidden">
                    {booking.package?.images?.[0]?.image_url ? (
                        <Image
                            src={booking.package.images[0].image_url}
                            alt={booking.package?.title || 'Booking Package'}
                            fill
                            ...
                            <div className="flex items-center gap-3">
                                ...
                                <div className="flex flex-wrap items-center gap-2">
                                    ...
                                    {(() => {
                                        ...
                                        if (isSplitAdvancePaid) {
                                            return (
                                                <div className="flex items-center gap-2 px-3 py-1 rounded-full border bg-purple-500/15 text-purple-600 border-purple-500/30 backdrop-blur-md shadow-sm ring-4 ring-white/10">
                                                    <CreditCard className="h-3.5 w-3.5" />
                                                    <span className="text-[10px] font-black uppercase tracking-widest">
                                                        {fps === 'PAID' ? '✅ Fully Paid' : fps === 'PENDING' ? '💳 Final Due' : '🔒 Advance Paid'}
                                                    </span>
                                                </div>
                                            );
                                        }
                                        if (booking.payment_status?.toLowerCase() !== 'paid' && booking.payment_status?.toLowerCase() !== 'succeeded') {
                                            return (
                                    ...
                                    {isPastTravel && booking.review_status && (
                                        <div className={`flex items-center gap-2 px-3 py-1 rounded-full border backdrop-blur-md shadow-sm ring-4 ring-white/10 ${booking.review_status === 'SUBMITTED'
                    ...
                    <div className="flex items-center justify-between mt-auto">
                        ...
                        <div className="flex items-center gap-8">
                            <div className="text-right">
                                <p className="text-[10px] font-black uppercase tracking-[0.15em] text-[var(--color-primary-font)]/50 mb-1">
                                    {(booking as any).is_split_payment ? 'Advance Paid' : 'Total Payment'}
                                </p>
                                <p className="text-2xl font-black text-[var(--primary)]">
                                    ₹{(booking as any).is_split_payment
                                        ? ((booking as any).advance_amount || booking.total_amount).toLocaleString()
                                        : booking.total_amount.toLocaleString()}
                                </p>
                                {(booking as any).is_split_payment && (booking as any).final_amount && (
                                    <p className="text-[9px] text-[var(--color-primary-font)]/50 font-bold mt-0.5">
                                ...
                                {isPastTravel && ['completed', 'confirmed'].includes(booking.status) && hasPermission('bookings', 'edit') && (
                                    booking.review_status !== 'SUBMITTED' && (
                                        <Button
                                            ...
                                            className={`h-12 px-6 rounded-full border backdrop-blur-md transition-all text-[11px] font-black uppercase tracking-widest gap-2 shadow-sm ${booking.review_status === 'SENT'
...
function BookingDetailsModal({ booking, isOpen, onClose, onBookingUpdated }: { booking: Booking | null, isOpen: boolean, onClose: () => void, onBookingUpdated?: (b: Booking) => void }) {
    ...
    const enableFinalPaymentMutation = useMutation({
        mutationFn: (bookingId: string) => bookingsAPI.enableFinalPayment(bookingId),
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ['agent-bookings'] })
            toast.success('Final payment unlocked and notification sent to customer!')
            if (booking && onBookingUpdated) {
                onBookingUpdated({ ...booking, final_payment_status: 'PENDING' } as any)
            }
        },
        onError: (error: any) => {
            toast.error(error.message || 'Failed to enable final payment')
        }
    })
                                    ...
                                    <div className="ml-auto">
                                        {booking.review_status === 'SUBMITTED' ? (
                                            ...
                                            <Button
                                                ...
                                                disabled={sendReviewMutation.isPending}
                                                className={`h-8 px-4 rounded-full border backdrop-blur-md transition-all text-[10px] font-black uppercase tracking-widest gap-2 ${booking.review_status === 'SENT'
                                    ...
                                    <div className="lg:col-span-12">
                                        <div className="space-y-5">
                                            ...
                                            <div className="pt-6 mt-6 border-t border-white/10 flex flex-col sm:flex-row justify-between items-end sm:items-center gap-6">
                                                <div className="flex flex-wrap items-center gap-4">
                                                    <div className="bg-white/5 px-4 py-2 rounded-2xl border border-white/10">
                                                        ...
                                                        <div className="flex items-center gap-2">
                                                            ...
                                                            <span className="text-sm font-black uppercase text-emerald-400 tracking-tighter">
                                                                {(booking as any).is_split_payment ? (booking as any).advance_payment_status || 'Advance' : booking.payment_status || 'Paid'}
                                                            </span>
                                                        </div>
                                                    </div>
                                                    <div className="bg-white/5 px-4 py-2 rounded-2xl border border-white/10">
                                                ...
                                                <div className="text-right">
                                                    <p className="text-[10px] font-black text-indigo-400 uppercase tracking-[0.3em] mb-2 leading-none">
                                                        {(booking as any).is_split_payment ? 'Advance Amount Paid' : 'Final Amount Paid'}
                                                    </p>
                                                    ...
                                                    {(booking as any).is_split_payment && (
                                                        <p className="text-[11px] text-slate-400 mt-1">Total: ₹{booking.total_amount.toLocaleString()}</p>
                                                    )}
                                                </div>
                                            </div>
                                            {/* Split Payment Schedule — Agent View */}
                                            {(booking as any).is_split_payment && (() => {
                                                const aps = (booking as any).advance_payment_status as string;
                                                const fps = (booking as any).final_payment_status as string;
                                                ...
                                                return (
                                                    <div className="mt-6 rounded-2xl overflow-hidden border border-white/10">
                                                        <div className="px-5 py-3 bg-white/5 border-b border-white/10">
                                                            <p className="text-[11px] font-black uppercase tracking-widest text-slate-400">Split Payment Schedule</p>
                                                        </div>
                                                        <div className="divide-y divide-white/5">
                                                            <div className="flex items-center justify-between px-5 py-4">
                                                                <div>
                                                                    <p className="text-[10px] uppercase tracking-wider text-slate-500 font-bold">Advance Paid</p>
                                                                    <p className="text-lg font-black text-white">₹{Number(advAmt).toLocaleString()}</p>
                                                                </div>
                                                                {aps === 'PAID' ? (
                                                                    <span className="px-3 py-1 rounded-full text-[10px] font-bold bg-emerald-900/50 text-emerald-300 border border-emerald-700/50">✅ Paid</span>
                                                                ) : (
                                                                    <span className="px-3 py-1 rounded-full text-[10px] font-bold bg-amber-900/50 text-amber-300 border border-amber-700/50">⏳ Pending</span>
                                                                )}
                                                            </div>
                                                            <div className="flex items-center justify-between px-5 py-4">
                                                                <div>
                                                                    <p className="text-[10px] uppercase tracking-wider text-slate-500 font-bold">Final Balance</p>
                                                                    <p className="text-lg font-black text-white">₹{Number(finalAmt).toLocaleString()}</p>
                                                                    {fps === 'PENDING' && dueDate && (
                                                                        <p className="text-[10px] text-sky-400 font-semibold mt-0.5">Due: {dueDate}</p>
                                                                    )}
                                                                </div>
                                                                {fps === 'PAID' && <span className="px-3 py-1 rounded-full text-[10px] font-bold bg-emerald-900/50 text-emerald-300 border border-emerald-700/50">✅ Paid</span>}
                                                                {fps === 'LOCKED' && (
                                                                    <div className="flex flex-col items-end gap-2">
                                                                        <span className="px-3 py-1 rounded-full text-[10px] font-bold bg-purple-900/50 text-purple-300 border border-purple-700/50">🔒 Awaiting Agent</span>
                                                                        <Button
                                                                            onClick={() => enableFinalPaymentMutation.mutate(booking.id)}
                                                                            disabled={enableFinalPaymentMutation.isPending || aps !== 'PAID'}
                                                                            className="h-8 px-4 bg-purple-500 hover:bg-purple-600 text-white text-[10px] font-bold rounded-lg disabled:opacity-50 disabled:cursor-not-allowed"
                                                                        >
                                                                            {enableFinalPaymentMutation.isPending ? <Loader2 className="h-3 w-3 animate-spin mr-1" /> : <Unlock className="h-3 w-3 mr-1" />}
                                                                            Approve Final Payment
                                                                        </Button>
                                                                    </div>
                                                                )}
                                                                {fps === 'PENDING' && <span className="px-3 py-1 rounded-full text-[10px] font-bold bg-sky-900/50 text-sky-300 border border-sky-700/50">⏳ Pending Customer</span>}
                                                            </div>
                                                        </div>
                                                    </div>
                                                );
