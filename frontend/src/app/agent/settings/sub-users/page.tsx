...
export default function SubUsersPage() {
    ...
    const handleResetPassword = async (id: string) => {
        toast("Reset Password?", {
            description: "A new temporary password will be emailed to them.",
            action: {
                label: "Reset",
                onClick: async () => {
                    try {
    ...
    return (
        <div className="p-6 max-w-7xl mx-auto space-y-6">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    <h1 className="text-2xl font-bold text-[var(--color-primary-font)] flex items-center gap-2">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="bg-white/40 backdrop-blur-md rounded-3xl p-5 border border-white/50 shadow-sm">
                    <p className="text-[10px] font-bold text-[var(--color-primary-font)]/40 uppercase tracking-widest leading-none mb-2">Total Staff</p>
                    <h3 className="text-3xl font-black text-[var(--color-primary-font)]">{subUsers.length}</h3>
                </div>
                <div className="bg-white/40 backdrop-blur-md rounded-3xl p-5 border border-white/50 shadow-sm">
                    <p className="text-[10px] font-bold text-[var(--color-primary-font)]/40 uppercase tracking-widest leading-none mb-2">Active Now</p>
                    <h3 className="text-3xl font-black text-green-600">{subUsers.filter(u => u.is_active).length}</h3>
                </div>
                <div className="bg-white/40 backdrop-blur-md rounded-3xl p-5 border border-white/50 shadow-sm">
                    <p className="text-[10px] font-bold text-[var(--color-primary-font)]/40 uppercase tracking-widest leading-none mb-2">Deactivated</p>
            ...
            <div className="bg-white/60 backdrop-blur-xl rounded-[32px] border border-white/40 shadow-xl overflow-hidden min-h-[400px]">
                <div className="p-6 border-b border-black/5 flex flex-col md:flex-row gap-4 items-center">
                    <div className="relative flex-1 w-full">
                        <Search className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-[var(--color-primary-font)]/30" />
                        <Input 
                            value={searchQuery}
                ...
                {loading ? (
                    <div className="flex flex-col items-center justify-center py-24 gap-4">
                        ...
                        <p className="text-sm font-medium text-[var(--color-primary-font)]/40">Loading staff directory...</p>
                    </div>
                ) : filteredUsers.length === 0 ? (
                    <div className="flex flex-col items-center justify-center py-24 px-6 text-center">
                        <div className="w-20 h-20 rounded-full bg-[var(--color-primary-font)]/5 flex items-center justify-center mb-4">
                            <Users className="w-10 h-10 text-[var(--color-primary-font)]/20" />
                        </div>
                        <h3 className="text-lg font-bold text-[var(--color-primary-font)]">No staff members found</h3>
                        <p className="text-sm text-[var(--color-primary-font)]/40 max-w-xs mt-1">
                    ...
                    <div className="overflow-x-auto">
                        <table className="w-full text-left">
                            <thead className="bg-[var(--color-primary-font)]/5 border-b border-black/5">
                                <tr className="text-[11px] font-bold text-[var(--color-primary-font)]/40 uppercase tracking-widest">
                            ...
                            <tbody className="divide-y divide-black/5">
                                {filteredUsers.map((user) => (
                                    <tr key={user.id} className="group hover:bg-[var(--color-primary-font)]/5 transition-colors">
                                        <td className="px-6 py-5">
                                            <div className="flex items-center gap-3">
                                                ...
                                                <div className="flex flex-col min-w-0">
                                                    <span className="text-sm font-bold text-[var(--color-primary-font)] leading-tight">
                                                        {user.first_name} {user.last_name}
                                                    </span>
                                                    <div className="flex items-center gap-2 mt-1">
                                                        <Mail className="w-3 h-3 text-[var(--color-primary-font)]/30" />
                                                        <span className="text-[11px] text-[var(--color-primary-font)]/40 font-medium truncate">{user.email}</span>
                                                    </div>
                                                </div>
                                            </div>
                                        </td>
                                        <td className="px-6 py-5">
                                            <span className="text-xs font-bold text-[var(--color-primary-font)]/70 bg-[var(--color-primary-font)]/5 px-2.5 py-1 rounded-lg">
                                                {user.role_label}
                                            </span>
                                        </td>
                                        <td className="px-6 py-5">
                                            <div className="flex flex-wrap gap-1">
                                                ...
                                                {(user.permissions?.length || 0) > 3 && (
                                                    <span className="text-[10px] text-[var(--color-primary-font)]/30 font-bold pl-1">+{user.permissions.length - 3} more</span>
                                                )}
                                            </div>
                                        </td>
                                        <td className="px-6 py-5">
                                            ...
                                            <DropdownMenu>
                                                <DropdownMenuTrigger asChild>
                                                    <Button variant="ghost" size="icon" className="h-8 w-8 rounded-full">
                                                        <MoreVertical className="w-4 h-4 text-[var(--color-primary-font)]/40" />
                                                    </Button>
                                                </DropdownMenuTrigger>
                                                <DropdownMenuContent align="end" className="rounded-2xl border-black/5 shadow-2xl p-1 w-48">
                                                    {hasPermission('settings', 'edit') ? (
                                                        ...
                                                        <DropdownMenuItem disabled className="text-[10px] font-bold uppercase text-[var(--color-primary-font)]/40 px-3">
                                                            View Only Mode
                                                        </DropdownMenuItem>
                                                    )}
                                                </DropdownMenuContent>
